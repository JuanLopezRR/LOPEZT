...
app.put('/api/citas/:id',(req,res)=>{
 ...
 db.run(
   'UPDATE citas SET nombre=COALESCE(?,nombre),negocio=COALESCE(?,negocio),telefono=COALESCE(?,telefono),correo=COALESCE(?,correo),plan=COALESCE(?,plan),fecha=COALESCE(?,fecha),hora=COALESCE(?,hora),notas=COALESCE(?,notas),estado=COALESCE(?,estado) WHERE id=?',
